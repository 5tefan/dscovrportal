...
def startof_3day(dt):
	...
	return datetime.utcfromtimestamp( ms_of_period )
...
def make_plot(output_path): ##can only be called after dscovr_ts_pane_config has been filled out with data
	...
	for pane_config in dscovr_ts_pane_config:
		...
		for stroke in pane_config[2]:
			...
			stroke.pop(), stroke.pop() ##get rid of the time and data from the end of the stroke config
...
def main():
	testdate = datetime.datetime(2015, 03, 20)
	...
	for frame_size in dscovr_ts_frame_sizes:							# loop over frame sizes defined
		...
		for pane_config in dscovr_ts_pane_config:
			...
			for stroke in pane_config[2]:
				...
				if frame_size[0] == "1month":
					...
					date_to_plot_millis = unix_time_millis( date_to_plot )
					start_millis = date_to_plot_millis + frame_size[3]
					end_millis = date_to_plot_millis + frame_size[4]
					
					current_get_millis = date_to_plot_millis
		...
		plot_filename = date_to_plot.strftime( frame_size[2] )
