...
/**
 * @ngdoc function
 * @name dscovrDataApp.controller:VisColorTsCtrl
 * @description
 * # VisColorTsCtrl
 * Controller of the dscovrDataApp
 */
angular.module('dscovrDataApp')
	.controller('VisColorTsCtrl', function ($scope, $timeout, $cookieStore,
	dscovrDataAccess, $routeParams, $location, $rootScope, $route) {

		// YOLO... DOM manipulation in the controller
		...
		var make_plot = function() {
			...
			$scope.pane_strs.split(";;").map( function(panel, panel_index) { if (panel) {
				...
				var _ = panel.split("*");
				lines = _[0].split(';').map( function(line) { 
					var __ = line.split(":");
					colors.push("#" + __[2]);
					return __.slice(0, 2).join(':');
				}).join(';');
				...
				dscovrDataAccess.getValues3(lines, timerange, conditions).then( function(data) {
					...
					var traces = y_accessor.map( function(att, j) { 
						return {
							x: [], y: [],
							mode: 'markers',
							marker: {
		...
		$scope.go = function() {
			evalSelections(function() {
				var new_url = "/vis/color/ts/" + $scope.pane_strs + "/" 
					+ $scope.timerange.join(":") + "/" +  $scope.condition_str;
				if ($location.url() != new_url) { // if location changed
					$cookieStore.put("cvis.arg", $scope.pane_strs);
					$cookieStore.put("cvis.argg", $scope.timerange.join(":"));
					$cookieStore.put("cvis.arggg", $scope.condition_str);
					if ($scope.can_plot) {
		...
		} else if ($cookieStore.get("cvis.arg")) {
			$scope.predef_param = $cookieStore.get("cvis.arg");
		};
		if ($routeParams.argg) { // the time range
			$scope.predef_time = $routeParams.argg.split(":").map( Number );
		} else if ($cookieStore.get("cvis.argg")) {
			$scope.predef_time = $cookieStore.get("cvis.argg").split(":").map( Number );
		};
		if ($routeParams.arggg) { // conditions
			$scope.predef_cond = $routeParams.arggg;
		} else if ($cookieStore.get("cvis.arggg")) {
			$scope.predef_cond = $cookieStore.get("cvis.arggg");
		};
		...
		$scope.$on('$locationChangeSuccess', function() {
			if ($route.current.$$route.controller !== 'VisColorTsCtrl') {
